...
function openDb(): Promise<IDBDatabase> {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => {
      const db = req.result;
      // Another tab wanting a newer schema blocks forever on a connection that never
...
export async function list(): Promise<RecordingListEntry[]> {
  // Clear the broken flag so this call retries; a failure below sets it back.
  ...
  if (isIndexedDbAvailable()) {
    try {
      ...
      // A failed read is not an empty library. The usual cause is a connection the
      // browser closed, and dbPromise would hand the dead handle to every later call.
      dbPromise = null;
      idbBroken = true;
      stored = [];
    }
...
export async function load(id: string): Promise<Recording | null> {
  if (isLegacyId(id)) {
    try {
      ...
      // Sanitized on the way out, as importFile does: an older dashboard wrote these
      // rows unbounded, so they are a route for an off-origin image path to the canvas.
...
/** Throws rather than reporting a read failure as absence: the caller stops the
 *  live session on a missing row, so folding a transient error into the same
 *  `null` would end a match in progress. */
...
export async function updateMeta(
  ...
  // One transaction, not load()-then-save(): the recorder's flush can land between
  // those awaits and the save would write back the older snapshot, truncating a run
  // renamed while it records. IndexedDB serialises overlapping readwrite txs.
  await tx([STORE_RECORDINGS, STORE_META], 'readwrite', ([recs, metas]) => {
...
/** Pinned recordings, legacy entries and anything currently loaded are never
 *  touched. */
...
/** idCounter is per-tab module state, so two dashboards on one robot both begin at
 *  1 and arm off the same status poll, landing in the same millisecond. */
const tabId = Math.random().toString(36).slice(2, 8);

export function newRecordingId(): string {
  idCounter += 1;
  // The counter is load-bearing: this tab can mint two ids in the same millisecond.
