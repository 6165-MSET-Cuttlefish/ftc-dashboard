...
const GamepadView: React.FC<GamepadViewProps> = ({
  ...
  const gamepadConnectionState = useSelector(
    (state: RootState) => state.gamepad,
  );
  const keyboardMappingState = useSelector(
  ...
  const keyboardTarget =
    selectedGamepad === 'both' ? keyboardTargetGamepad : selectedGamepad;
  ...
  const currentGamepadState =
    selectedGamepad === 1 ? gamepad1State : gamepad2State;
  ...
  return (
    <BaseView isUnlocked={isUnlocked}>
      <div className="flex">
        <BaseViewHeading isDraggable={isDraggable}>Gamepad</BaseViewHeading>
        <BaseViewIcons>
          <BaseViewIconButton
      ...
      <BaseViewBody className="space-y-3 p-4">
        {/* Simplified Toolbar */}
        ...
        {selectedGamepad === 'both' &&
          keyboardMappingState.enabled &&
          !anyHardwareConnected && (
            <div className="flex items-center gap-2 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 dark:border-blue-800 dark:bg-blue-900/20">
              <GamepadIcon className="h-4 w-4 flex-shrink-0 text-blue-600 dark:text-blue-400" />
        ...
        {anyHardwareConnected && (
          <div className="flex items-center gap-2 rounded-md border border-green-200 bg-green-50 px-3 py-2 dark:border-green-800 dark:bg-green-900/20">
            <span className="text-xs font-medium text-green-700 dark:text-green-300">
              ● Hardware gamepad connected - UI and keyboard controls are
              disabled
            </span>
          </div>
        )}
        ...
        {selectedGamepad === 'both' ? (
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            <div>
              <h3 className="mb-2.5 text-sm font-semibold text-gray-700 dark:text-gray-300">
                Gamepad 1
              </h3>
              <GamepadControls
                gamepadNum={1}
