...
// buildFrame returns null when a batch says nothing about telemetry.
...
// Only DashboardTelemetry marks its packets, so this is a frame from the Telemetry adapter.
...
describe('batching', () => {
  ...
  it('drops telemetry that is no longer being sent', () => {
    // Each batch is built from scratch, so nothing lingers until telemetry is cleared.
    expect(
  ...
  it('keeps a key dropped mid-batch until the batch after it', () => {
    // Within a batch, a key dropped by the next loop and a key from another source look alike.
    expect(
      rendered([
  ...
  it('keeps the bare lines of every distinct frame in a batch', () => {
    // An action packet and the op mode's own telemetry, both sent each loop.
    expect(
      rendered([
  ...
  it('does not duplicate a line that only appears on some loops', () => {
    // A conditional warning changes a frame's length but not its captions, which identify it.
    expect(
      rendered([
  ...
  it('collapses a repeated frame whose bare line text keeps changing', () => {
    // Only the captions form a frame's shape, so a line's text may change freely.
    expect(
      rendered([
  ...
  it('clears the display one batch after telemetry.clear()', () => {
    // Within a batch an empty packet is indistinguishable from one that only drew something.
    expect(
  ...
  it('holds telemetry through a batch of only drawing packets', () => {
    // DashboardCore strips the ops from all but the newest, so the rest arrive looking empty.
  ...
  // With no caption to match on, frames are told apart by timestamp: packets queued in one loop
  // iteration share one, a source resending each loop does not. Both frames have one line on
  // purpose: length alone could not separate them.
  ...
  it('separates op mode telemetry from a hand-built packet without relying on the clock', () => {
    // The adapter's frames are identified by source, so the clock cannot confuse them.
    expect(
      rendered([
...
describe('log', () => {
  ...
  it('clears one batch after the robot stops sending it', () => {
    // The adapter retransmits the whole log every packet, so no log at all means it was cleared.
...
describe('display format', () => {
  it('assumes HTML for a packet from a robot too old to declare a format', () => {
    // That version rendered markup unconditionally; CLASSIC would newly show tags as literal text.
  ...
  it('follows each line rather than the view', () => {
    // A path follower's packets and the op mode's telemetry need not agree on a format.
    const built = buildFrame([
  ...
  it('is not reset by a packet that only carries a field overlay', () => {
    // An overlay-only packet contributes no lines, so its format has nothing to apply to.
    const built = buildFrame([
...
describe('packets from an older robot', () => {
  ...
  it('tolerates malformed entries', () => {
    ...
    // A non-string caption becomes a bare line and a non-string value an empty one.
