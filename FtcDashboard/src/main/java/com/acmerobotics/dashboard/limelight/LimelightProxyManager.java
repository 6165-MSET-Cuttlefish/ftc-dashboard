...
/**
 * Opens a set of local servers on the Control Hub that forward traffic to the Limelight 3A sitting
 * on the internal USB-Ethernet network at 172.29.0.1.
 *
 * <p>The Limelight exposes several services the dashboard client needs:
 *
 * <ul>
 *   <li><b>5800</b> – MJPEG camera stream (HTTP, chunked)
 *   <li><b>5801</b> – Web dashboard UI (HTTP)
 *   <li><b>5805</b> – WebSocket interface (raw TCP)
 *   <li><b>5807</b> – REST API for status &amp; config (HTTP)
 * </ul>
 *
 * Because the Limelight's subnet isn't routable from the browser, these forwarders let the client
 * reach every service at the same port numbers on the Control Hub's Wi-Fi address.
 */
public class LimelightProxyManager {
    ...
    public synchronized void stop() {
        ...
        if (mjpegForwarder != null) {
            mjpegForwarder.stop();
        }
        if (dashForwarder != null) {
            dashForwarder.stop();
        }
        if (wsForwarder != null) {
            wsForwarder.close();
        }
        if (apiForwarder != null) {
            apiForwarder.stop();
        }
