...
/**
 * Hardware operation mode that provides real-time hardware configuration and monitoring through the
 * FTC Dashboard interface. This OpMode automatically discovers motors and servos in the hardware
 * map and exposes their configuration and telemetry data through the dashboard.
 *
 * <p>Features include:
 *
 * <ul>
 *   <li>Real-time motor power control and position monitoring
 *   <li>Motor current draw monitoring
 *   <li>Servo position control
 *   <li>Hardware port information display
 *   <li>Dynamic hardware configuration updates
 * </ul>
 */
...
public class HardwareOpMode extends OpMode {
    ...
    /**
     * Creates a ConfigVariable from a given value using a VariableProvider. This is a utility
     * method for wrapping simple values in the dashboard's variable system.
     *
    ...
    public static <T> ConfigVariable<T> createVariableFromValue(
            VariableType variableType, T value) {
        return new BasicVariable<>(variableType, new VariableProvider<>(value));
    }

    /**
     * Initializes the hardware monitoring system by setting up the dashboard hardware root. This
     * method is called once when the OpMode is initialized.
     */
    @Override
    public void init() {
    ...
    /**
     * Continuously updates hardware configuration and telemetry during the init phase. This method
     * runs in a loop after init() is called but before start() is called. Includes a 100ms sleep to
    ...
    /**
     * Main loop that continuously updates hardware configuration and telemetry data. This method
    ...
    /**
     * Initializes all hardware components by setting up their dashboard variables. This method
     * discovers and configures both motors and servos.
     *
    ...
    /**
     * Applies configuration changes from the dashboard to the actual hardware devices. This method
     * updates both motors and servos based on dashboard inputs.
     *
     * @param hardwareRoot the root variable container for hardware components
     */
    ...
    /**
     * Updates the dashboard with current hardware state information. This method refreshes
     * telemetry data like motor positions and current draw.
     *
     * @param hardwareRoot the root variable container for hardware components
     */
    ...
    /**
     * Discovers all motors in the hardware map and creates dashboard variables for them. Each motor
     * gets variables for power, position, run mode, current draw, and port information.
     *
    ...
    /**
     * Creates a complete dashboard variable structure for a single motor. Includes power control,
     * position monitoring, run mode, current draw, and port info.
     *
    ...
    private CustomVariable createMotorVariable(DcMotorEx motor) {
        ...
        motorVar.putVariable(
                ...
                createVariableFromValue(
                        VariableType.READONLY_STRING, String.valueOf(motor.getCurrentPosition())));
        motorVar.putVariable("Target Position", createVariableFromValue(motor.getTargetPosition()));
        motorVar.putVariable(
                "Run Mode", createVariableFromValue(DcMotorEx.RunMode.RUN_WITHOUT_ENCODER));
        ...
        motorVar.putVariable(
                hubType + " Port",
                createVariableFromValue(
                        VariableType.READONLY_STRING, String.valueOf(motor.getPortNumber())));

        motorVar.putVariable(
                "Velocity",
                createVariableFromValue(
                        VariableType.READONLY_STRING, String.valueOf(motor.getVelocity())));

        return motorVar;
    }

    /**
     * Applies configuration changes from the dashboard to all motors. Updates motor run modes and
     * target positions based on dashboard inputs.
     *
    ...
    /**
     * Applies configuration settings from the dashboard to a specific motor. Handles run mode
     * changes and target position updates with error handling.
     *
    ...
    /**
     * Updates the dashboard with current motor telemetry data. Refreshes current position and
     * current draw readings for all motors.
     *
    ...
    private void updateMotorStateVariables(CustomVariable hardwareRoot) {
        ...
        for (DcMotorSimple motor : hardwareMap.getAll(DcMotorSimple.class)) {
            ...
            stateUpdate.putVariable(
                    "Current Position",
                    createVariableFromValue(
                            VariableType.READONLY_STRING,
                            String.valueOf(motorEx.getCurrentPosition())));
            ...
            stateUpdate.putVariable(
                    "Velocity",
                    createVariableFromValue(
                            VariableType.READONLY_STRING, String.valueOf(motorEx.getVelocity())));
    ...
    /**
     * Discovers all servos in the hardware map and creates dashboard variables for them. Each servo
     * gets variables for position control and port information.
     *
    ...
    /**
     * Creates a dashboard variable structure for a single servo. Includes position control and port
     * information.
     *
    ...
    private CustomVariable createServoVariable(Servo servo) {
        ...
        servoVar.putVariable(
                hubType + " Port",
                createVariableFromValue(
                        VariableType.READONLY_STRING, String.valueOf(servo.getPortNumber())));

        return servoVar;
    }

    /**
     * Applies configuration changes from the dashboard to all servos. Updates servo positions based
     * on dashboard inputs.
     *
    ...
    /**
     * Applies position configuration from the dashboard to a specific servo. Uses -1.0 as a
     * sentinel value to indicate no position change is desired.
     *
    ...
    /**
     * Discovers all CRServos in the hardware map and creates dashboard variables for them. Each
     * motor gets variables for power and port information.
     *
    ...
    /**
     * Creates a complete dashboard variable structure for a single CR Servo. Includes power control
    ...
    /**
     * Applies configuration changes from the dashboard to all CRServos. Updates CRServo powers
     * based on dashboard inputs.
     *
    ...
    private void updateCrServoFromConfig(CustomVariable hardwareRoot) {
        CustomVariable crServosVar =
                (CustomVariable) hardwareRoot.getVariable("Continuous Rotation Servos");
        if (crServosVar == null) {
            return;
        }
    ...
    /**
     * Applies configuration settings from the dashboard to a specific motor. Handles power changes.
     *
    ...
    /**
     * Creates a complete dashboard variable structure for a single color sensor. Includes RGB and
     * port information.
     *
    ...
    private CustomVariable createColorSensorVariable(ColorSensor colorSensor) {
        ...
        colorSensorVar.putVariable(
                hubType + " Port",
                createVariableFromValue(
                        VariableType.READONLY_STRING,
                        extractI2CPort(colorSensor.getConnectionInfo())));

        return colorSensorVar;
    }

    /**
     * Updates the dashboard with current color sensor telemetry data. Refreshes color readings for
     * all color sensors.
     *
    ...
    private void updateColorSensorStateVariables(CustomVariable hardwareRoot) {
        ...
        for (ColorSensor colorSensor : hardwareMap.getAll(ColorSensor.class)) {
            ...
            CustomVariable existingConfig =
                    (CustomVariable) colorSensorsVar.getVariable(deviceName);
            if (existingConfig != null) {
                existingConfig.update(stateUpdate);
            }
        }
    ...
    private void getColorSensorState(ColorSensor colorSensor, CustomVariable stateVariable) {
        stateVariable.putVariable(
                "Red",
                createVariableFromValue(
                        VariableType.READONLY_STRING, String.valueOf(colorSensor.red())));
        stateVariable.putVariable(
                "Green",
                createVariableFromValue(
                        VariableType.READONLY_STRING, String.valueOf(colorSensor.green())));
        stateVariable.putVariable(
                "Blue",
                createVariableFromValue(
        ...
        if (colorSensor instanceof NormalizedColorSensor) {
            ...
            stateVariable.putVariable(
                    "Normalized Red",
                    createVariableFromValue(
                            VariableType.READONLY_STRING, String.valueOf(reading.red)));
            stateVariable.putVariable(
                    "Normalized Green",
                    createVariableFromValue(
                            VariableType.READONLY_STRING, String.valueOf(reading.green)));
            stateVariable.putVariable(
                    "Normalized Blue",
                    createVariableFromValue(
    ...
    /**
     * Retrieves the configuration name of a hardware device from the hardware map. Handles cases
     * where a device might have multiple names by returning the first one.
     *
    ...
    /**
     * Extracts and formats the hub type from a device's connection information string. Determines
     * whether a device is connected to a Control Hub or Expansion Hub based on the numeric
     * identifier in the connection info.
     *
