...
public class TestFieldVersatilityOpMode extends TestOpMode {
    ...
    protected void init() {
        ...
        dashboard.core.addConfigVariable(
                "Test",
                "ORIGIN_HEADING_OFFSET",
    ...
    protected void loop() throws InterruptedException {
        ...
        // drawing an orbiting triangle pointing mostly up the X axis to indicate field theta = 15
        ...
        packet.fieldOverlay()
                ...
                // images, text and grids by default are drawn using the page transform where 0,0 is
                // top left and 144,144 is bottom right
                // but extra parameters are available to allow rotation and drawing in the current
                // transform
                // all other drawing primitives are rendered in the current (or default) transform,
                // as built using setTranslation, setRotation and setScale
                .setAlpha(.25)
                .drawImage(ALTIMGSRC, ALTIMGX, ALTIMGY, ALTIMGW, ALTIMGH)

                // optionally add sets of custom gridlines, minimum of 2 to render field edges,
                // anything less suppresses gridlines in that direction, default is 7
                .setAlpha(1.0)
                ...
                .drawGrid(
                        ...
                        GRID_PIVOTY,
                        GRID_USE_PAGE_FRAME)
                // .drawGrid(GRIDX, GRIDY, GRIDW, GRIDH, GRID_LINESX, GRID_LINESY, angleAnim,
                // GRID_PIVOTX, GRID_PIVOTY, GRID_USE_PAGE_FRAME)

                // you can draw multiple images and can rotate them around a specified anchor/pivot
                // point and draw them in the current transform instead of the page frame
                .drawImage("/dash/decode.webp", 24, 24, 48, 48, 0, 0, 0, false)

                // demonstrate an alternate transform to move the origin and orientation
                // default origin for dashboard is in the center of the field with X axis pointing
                // up
                // for powerplay season iron reign decided to set the origin to the alliance
                // substation
                ...
                // .setRotation(-Math.PI/4) //uncomment to see a rotation of 45 degrees, there have
                // been FTC games with a diagonal field symmetry
                ...
                // the text labels will be drawn in the current origin transform, not in the page
                // frame
                .setStrokeWidth(1)
                .setStroke("green")
                ...
                .setAlpha(.25)
                // you can draw multiple images and can rotate them around a specified pivot point,
                // and draw them in the current transform instead of the page frame
